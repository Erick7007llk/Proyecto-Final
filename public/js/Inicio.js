...
function addToCart(id, name, price, stock) {
  ...
  if (existing) {
    if (maxStock && existing.qty >= maxStock) {
      showToast(`Solo hay ${maxStock} unidades disponibles`, "error");
      return;
    }
    existing.qty++;
  } else {
    cart.push({ id: productId, name, price: Number(price), qty: 1, stock: maxStock || null });
  }
  saveCart();
...
function changeQty(index, delta) {
  ...
  if (delta > 0 && maxStock && item.qty >= maxStock) {
    showToast(`Solo hay ${maxStock} unidades disponibles`, "error");
    return;
  }
  item.qty += delta;
  if (item.qty <= 0) return removeFromCart(index);
  saveCart();
}
...
async function checkout() {
  ...
  if (cart.some(item => !item.id)) {
    showToast("Tu carrito tiene productos desactualizados. Vacíalo y vuelve a agregar.", "error");
    return;
  }
...
async function submitOrder() {
  ...
  try {
    const res = await fetch("/api/pedido", {
      ...
      body: JSON.stringify({ items: cart, total, direccion: direccion.trim() }),
    });
